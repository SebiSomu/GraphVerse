...
VisSupermarket::VisSupermarket(QWidget *parent)
    : QWidget(parent), m_graph(nullptr), m_startIdx(-1), m_endIdx(-1),
      m_finalPath(), m_pathCost(0), m_toolbar(nullptr),
...
void VisSupermarket::hideEvent(QHideEvent *event) {
  onResetClicked();
...
void VisSupermarket::mouseReleaseEvent(QMouseEvent *e) {
  ...
  if (!m_isDragging) {
    ...
    if (m_startIdx != -1 && m_endIdx != -1) {
      ...
      QString endName = "...";
      for (const auto &n : m_graph->getNodes()) {
        if (n.getIndex() == m_startIdx)
          startName = n.getName();
        if (n.getIndex() == m_endIdx)
          endName = n.getName();
      }
...
void VisSupermarket::paintEvent(QPaintEvent *) {
  ...
  if (!m_finalPath.empty()) {
    ...
    for (const auto &node : m_graph->getNodes()) {
      bool isSpecial =
          (node.getIndex() == m_startIdx || node.getIndex() == m_endIdx);
      RenderSettings s = settings;
      if (isSpecial) {
        s.highlightColor = (node.getIndex() == m_startIdx)
                               ? QColor(34, 197, 94)
                               : QColor(239, 68, 68);
