...
void AnimatedButton::paintEvent(QPaintEvent *) {
  ...
  QColor c1 = m_glowColor;
  c1.setAlphaF(0.18f + 0.12f * m_glowOpacity);
  QColor c2 = m_glowColor;
  c2.setAlphaF(0.06f + 0.06f * m_glowOpacity);
  bg.setColorAt(0, c1);
  bg.setColorAt(1, c2);
  p.fillPath(path, bg);
...
// ─────────────────────────────────────────────────────────────
BigCategoryButton::BigCategoryButton(const QString &title,
...
void BigCategoryButton::paintEvent(QPaintEvent *) {
  ...
  QColor acc0 = m_accentColor;
  acc0.setAlphaF(0.0f);
  QColor acc1 = m_accentColor;
  acc1.setAlphaF(m_hovered ? 0.9f : 0.55f);
  stripGrad.setColorAt(0.0, acc0);
  stripGrad.setColorAt(0.5, acc1);
  stripGrad.setColorAt(1.0, acc0);
  p.fillRect(QRectF(full.x() + 2, full.y() + 2, full.width() - 4, 3),
             stripGrad);

  if (m_hovered) {
    QRadialGradient glow(full.center().x(), 0, full.width() * 0.7f);
    QColor ga = m_accentColor;
    ga.setAlphaF(0.09f);
    QColor gb = m_accentColor;
    gb.setAlphaF(0.0f);
  ...
  QColor circFill = m_accentColor;
  circFill.setAlphaF(0.15f);
  p.fillPath(circlePath, circFill);
  QColor circBorder = m_accentColor;
  circBorder.setAlphaF(0.3f);
...
// ─────────────────────────────────────────────────────────────
GraphVersePlatform::GraphVersePlatform(QWidget *parent) : QMainWindow(parent) {
  applyGlobalStyle();
  setupUi();
  resize(980, 720);
...
QWidget *GraphVersePlatform::buildHeader() {
  ...
  nameLabel->setStyleSheet(
      "font: 700 18px 'Segoe UI'; color:#EBEDFF; letter-spacing:1px;");
  auto *tagLabel = new QLabel("Platform");
  tagLabel->setStyleSheet(
      "font: 400 11px 'Segoe UI'; color:#6366F1; margin-top:6px;");
  ...
  homeLnk->setStyleSheet(
      "QPushButton { font: 500 11px 'Segoe UI'; color:#8C94BE; border:none; "
...
QWidget *GraphVersePlatform::buildFooter() {
  ...
  footer->setStyleSheet("background: rgba(8,10,20,0.95); border-top: 1px solid "
                        "rgba(45,55,100,0.5);");
...
HeroWidget *GraphVersePlatform::buildHeroSection() {
  ...
  title->setStyleSheet(
      "font: 700 42px 'Segoe UI'; color: #EBEDFF; line-height: 1.15;");
  title->setAlignment(Qt::AlignCenter);

  auto *sub =
      new QLabel("Select a path below to discover our tools and applications.");
...
QWidget *GraphVersePlatform::buildHomeView() {
  ...
  lbl->setStyleSheet(
      "font: 700 13px 'Segoe UI'; color:#8C94BE; letter-spacing:2px;");
  lbl->setAlignment(Qt::AlignCenter);
  clay->addWidget(lbl);

  auto *btnVis = new BigCategoryButton(
...
static QWidget *buildPageHeader(const QString &title, const QString &icon,
  ...
  QObject::connect(btnBack, &QPushButton::clicked, stack,
  ...
  titleLbl->setStyleSheet(
      QString("font: 700 28px 'Segoe UI'; color: %1;").arg(accent.name()));
...
QWidget *GraphVersePlatform::buildPlaceholderView() {
  ...
  m_placeholderTitleLabel->setStyleSheet(
      "font: 700 28px 'Segoe UI'; color: #EBEDFF;");
  ...
  auto *placeholderMsg =
      new QLabel("This module is currently being calibrated.\nCheck back soon "
...
void GraphVersePlatform::setupUi() {
  ...
  auto addAppPage = [&](QWidget *appWidget) {
    ...
    wlay->setContentsMargins(0, 0, 0, 0);
    wlay->setSpacing(0);
    auto *backBtn = new AnimatedButton("← Back to Applications");
    backBtn->setGlowColor(QColor(140, 148, 190));
    backBtn->setMinimumHeight(40);
    backBtn->setMaximumHeight(40);
    backBtn->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
