...
Edge::Edge(Node* f, Node* s, int capacity)
    : m_first(f), m_second(s), m_capacity(capacity), m_flow(0), m_isMinCut(false) {}
...
bool Edge::isMinCut() const {
    return m_isMinCut;
}

void Edge::setMinCut(bool minCut) {
    m_isMinCut = minCut;
}
