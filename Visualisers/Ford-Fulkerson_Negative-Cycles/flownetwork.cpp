...
FlowNetwork::FlowNetwork()
    : m_source(-1), m_sink(-1), m_currentTotalFlow(0)
{
}
...
void FlowNetwork::addEdge(int from, int to, int capacity) {
    int u = from - 1, v = to - 1;
    if (u >= 0 && u < (int)m_nodePositions.size() &&
        v >= 0 && v < (int)m_nodePositions.size())
...
int FlowNetwork::getEdgeCost(int from, int to) const {
    int u = from - 1, v = to - 1;
    if (u >= 0 && u < (int)m_cost.size() && v >= 0 && v < (int)m_cost.size())
        return m_cost[u][v];
    return 0;
}
...
void FlowNetwork::initializeAlgorithm() {
    resetFlow();
    m_currentResidual = m_capacity;

    IterationState s;
    s.iterationNumber = 0;
    s.residualCapacity = m_currentResidual;
    s.pathFlow = 0;
    s.totalFlow = 0;
    s.description = "Initial state. Total flow: 0";
...
bool FlowNetwork::performStep() {
    ...
    if (bfs(m_currentResidual, s, t, parent)) {
        ...
        while (v != s) {
            path.push_back(v + 1);
            int u = parent[v];
            pathFlow = std::min(pathFlow, m_currentResidual[u][v]);
            v = u;
        }
        path.push_back(s + 1);
        ...
        while (v != s) {
            int u = parent[v];
            m_currentResidual[u][v] -= pathFlow;
            m_currentResidual[v][u] += pathFlow;
            v = u;
        }
        m_currentTotalFlow += pathFlow;

        IterationState state;
        state.iterationNumber = m_iterations.size();
        state.residualCapacity = m_currentResidual;
        state.augmentingPath = path;
        state.pathFlow = pathFlow;
        state.totalFlow = m_currentTotalFlow;
...
void FlowNetwork::finalizeMinCut() {
    ...
    while (!q.empty()) {
        int u = q.front(); q.pop();
        for (int v = 0; v < n; v++)
            if (!visited[v] && m_currentResidual[u][v] > 0) {
                visited[v] = true;
                q.push(v);
            }
    }
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            if (visited[i] && !visited[j] && m_capacity[i][j] > 0)
                m_minCutEdges.insert({i + 1, j + 1});
}
...
int FlowNetwork::eliminateNegativeCycles() {
    ...
    bool hasCosts = false;
    for (int i = 0; i < n && !hasCosts; i++)
        for (int j = 0; j < n && !hasCosts; j++)
            if (m_cost[i][j] != 0) hasCosts = true;
    if (!hasCosts) return 0;

    if ((int)m_currentResidual.size() != n)
        m_currentResidual = m_capacity;
    ...
    for (int guard = 0; guard < 1000; guard++) {
        ...
        for (int i = 0; i + 1 < (int)cycle.size(); i++) {
            int u = cycle[i] - 1, v = cycle[i+1] - 1;
            r = std::min(r, m_currentResidual[u][v]);
        }
        if (r <= 0 || r == INT_MAX) break;

        for (int i = 0; i + 1 < (int)cycle.size(); i++) {
            int u = cycle[i] - 1, v = cycle[i+1] - 1;
            m_currentResidual[u][v] -= r;
            m_currentResidual[v][u] += r;
        }
        eliminated++;
    }
...
int FlowNetwork::getCapacity(int from, int to) const {
    int u = from-1, v = to-1;
    if (u >= 0 && u < (int)m_capacity.size() && v >= 0 && v < (int)m_capacity.size())
        return m_capacity[u][v];
    return 0;
}

int FlowNetwork::getResidual(int from, int to) const {
    int u = from-1, v = to-1;
    if (u >= 0 && u < (int)m_currentResidual.size() && v >= 0 && v < (int)m_currentResidual.size())
        return m_currentResidual[u][v];
    return 0;
}
...
int FlowNetwork::getFlow(int from, int to) const {
    ...
    if (m_capacity[u][v] > 0)
        return m_capacity[u][v] - m_currentResidual[u][v];
    return 0;
}
...
bool FlowNetwork::isMinCutEdge(int from, int to) const {
    return m_minCutEdges.find({from, to}) != m_minCutEdges.end();
}
