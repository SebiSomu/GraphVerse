    ...
    , m_currentAlgo(AlgoType::None)
{
    ...
    QPushButton* btnB = new QPushButton("Boruvka", this);
    QPushButton* btnR = new QPushButton("Reset",   this);

    for(auto* btn : {btnK, btnP, btnB, btnR}) {
        btn->setFixedHeight(34);
        btn->setFixedWidth(110);
    }

    QString base = "QPushButton{border-radius:6px;font-weight:bold;font-size:12px;}";
    btnK->setStyleSheet(base + "QPushButton{background:#1e90ff;color:white;}"
                               "QPushButton:hover{background:#1070d0;}");
    btnP->setStyleSheet(base + "QPushButton{background:#27ae60;color:white;}"
                               "QPushButton:hover{background:#1e8449;}");
    btnB->setStyleSheet(base + "QPushButton{background:#9b59b6;color:white;}"
    ...
    connect(btnK, &QPushButton::clicked, this, &MainWindow::onKruskalClicked);
...
void MainWindow::onKruskalClicked() { startAnimation(AlgoType::Kruskal); }
void MainWindow::onPrimClicked()    { startAnimation(AlgoType::Prim);    }
void MainWindow::onBoruvkaClicked() { startAnimation(AlgoType::Boruvka); }
...
void MainWindow::paintEvent(QPaintEvent*)
{
    ...
    auto nodePos = [&](int idx) -> QPointF {
        ...
        for(const auto& n : nodes)
            if(n.getIndex() == idx)
                return QPointF(n.getX(), n.getY());
        return QPointF(0,0);
    };

    auto drawEdge = [&](const MSTStep& s, QColor col, int width, bool showCost) {
        ...
        if(showCost) {
            ...
            p.save();
            QFont f = p.font(); f.setPointSize(7); f.setBold(true);
            p.setFont(f);
            p.setPen(col);
            p.drawText(mid + off, QString::number(s.cost));
            p.restore();
        }
    };
    ...
    for(const auto& n : m_graph->getNodes()) {
        ...
        bool inMST = false;
        for(const auto& s : m_accepted)
            if(s.fromIndex == n.getIndex() || s.toIndex == n.getIndex())
            { inMST = true; break; }

        QColor fill = inMST
                          ? (m_animDone ? QColor(240,240,240) : QColor(0,180,60))
                          : QColor(50, 50, 60);
        QColor border = inMST
                            ? (m_animDone ? Qt::white : QColor(0,220,80))
                            : QColor(130,130,150);
        ...
        QFont f = p.font(); f.setPointSize(6); f.setBold(false);
        p.setFont(f);
    ...
    if(m_currentAlgo != AlgoType::None) {
        ...
        QFont f = p.font(); f.setPointSize(11); f.setBold(true);
        p.setFont(f);
        p.setPen(algoCol);

        int totalCost = 0;
        for(const auto& s : m_accepted) totalCost += s.cost;

        QString status = m_animDone
                             ? QString("%1 - done!  MST edges: %2  Total cost: %3")
                                   .arg(algoName).arg(m_accepted.size()).arg(totalCost)
                             : QString("%1 - building MST...  edges added: %2")
                                   .arg(algoName).arg(m_accepted.size());
