    ...
    , m_graph(nullptr)
{
    ...
    btnBfs->setStyleSheet(base +
                          "QPushButton { background:#1e90ff; color:white; }"
                          "QPushButton:hover { background:#1070d0; }");
    btnDfs->setStyleSheet(base +
                          "QPushButton { background:#27ae60; color:white; }"
                          "QPushButton:hover { background:#1e8449; }");
    btnReset->setStyleSheet(base +
...
void MainWindow::generateMaze()
{
    ...
    for(int r = 0; r < ROWS; r++)
        for(int c = 0; c < COLS; c++)
            m_graph->addNode(QPoint(offsetX() + c * CELL + CELL/2,
    ...
    for(int r = 0; r < ROWS; r++) {
        for(int c = 0; c < COLS; c++) {
            for(int d = 0; d < 4; d++) {
                ...
                int nr = r + DR[d], nc = c + DC[d];
                int idxA = cellToIndex(r,  c,  COLS) - 1;
                int idxB = cellToIndex(nr, nc, COLS) - 1;
                m_graph->addEdge(nodes[idxA], nodes[idxB]);
            }
        }
    }
...
static void reconstructPath(int endIdx, int cols,
...
std::vector<std::pair<int,int>> MainWindow::runBFS(std::vector<std::pair<int,int>>& outPath)
{
    ...
    std::vector<int> order = m_graph->bfs(
        startIdx, endIdx,
        nullptr,
        [&](int par, int child){ parent[child] = par; }
        );

    std::vector<std::pair<int,int>> result;
    for(int idx : order)
        result.push_back({indexToRow(idx, COLS), indexToCol(idx, COLS)});
...
std::vector<std::pair<int,int>> MainWindow::runDFS(std::vector<std::pair<int,int>>& outPath)
{
    ...
    std::vector<int> order = m_graph->dfs(
        startIdx, endIdx,
        nullptr,
        [&](int par, int child){ parent[child] = par; }
        );
...
void MainWindow::startAnimation(AlgoType algo)
{
    ...
    if(algo == AlgoType::BFS)
        ...
        m_visitOrder = runDFS(m_finalPath);
...
void MainWindow::onAnimationTick()
{
    if(!m_showingPath) {
        if(m_animStep < (int)m_visitOrder.size()) {
            auto [r, c] = m_visitOrder[m_animStep++];
            m_visited[r][c] = true;
            update();
        } else {
            m_showingPath = true;
        ...
        if(m_pathStep < (int)m_finalPath.size()) {
            m_pathHighlight.push_back(m_finalPath[m_pathStep++]);
            update();
        } else {
            m_timer->stop();
            m_animDone = true;
...
void MainWindow::onBfsClicked()   { startAnimation(AlgoType::BFS); }
void MainWindow::onDfsClicked()   { startAnimation(AlgoType::DFS); }
void MainWindow::onResetClicked() { m_timer->stop(); generateMaze(); }
...
void MainWindow::paintEvent(QPaintEvent*)
{
    ...
    const int ox = offsetX(), oy = offsetY(), C = CELL;

    // Pure black background
    p.fillRect(rect(), Qt::black);

    QColor visitColor = (m_currentAlgo == AlgoType::BFS)
                            ? QColor(30, 100, 200, 180)
                            : QColor(39, 174,  96, 180);

    for(int r = 0; r < ROWS; r++)
        for(int c = 0; c < COLS; c++)
            if(m_visited[r][c])
                p.fillRect(ox + c*C + 1, oy + r*C + 1, C-2, C-2, visitColor);

    for(auto& [r, c] : m_pathHighlight)
        p.fillRect(ox + c*C + 3, oy + r*C + 3, C-6, C-6, QColor(255, 220, 0));

    p.setPen(QPen(QColor(190, 200, 220), 2));
    for(int r = 0; r < ROWS; r++) {
        for(int c = 0; c < COLS; c++) {
            int x = ox + c*C, y = oy + r*C;
            if(m_maze[r][c].walls[0]) p.drawLine(x,   y,   x+C, y  ); // N
            if(m_maze[r][c].walls[1]) p.drawLine(x+C, y,   x+C, y+C); // E
            if(m_maze[r][c].walls[2]) p.drawLine(x,   y+C, x+C, y+C); // S
            if(m_maze[r][c].walls[3]) p.drawLine(x,   y,   x,   y+C); // W
        }
    }
    ...
    if(m_currentAlgo != AlgoType::None) {
        ...
        QColor  algoCol  = (m_currentAlgo == AlgoType::BFS)
                             ? QColor(80, 160, 255) : QColor(46, 204, 113);
        ...
        QString status = m_animDone
                             ? QString("%1 - done!  Path length: %2 steps").arg(algoName).arg(m_finalPath.size())
                             : QString("%1 - searching for exit...").arg(algoName);

        p.drawText(QRect(ox, oy + ROWS*C + 6, COLS*C, 26),
